...
class ISpout(object):
    """ ISpout is the core interface for implementing spouts. A Spout is
    responsible for feeding messages into the topology for processing.
    For every tuple emitted by a spout, Storm will track the (potentially
    very large) DAG of tuples generated based on a tuple emitted by the spout.
    When Storm detects that every tuple in that DAG has been successfully
    processed, it will send an ack message to the Spout.

    If a tuple fails to be fully processed within the configured timeout for
    the topology, Storm will send a fail message to the spout for the message.

    When a Spout emits a tuple, it can tag the tuple with a message id.
    The message id can be any type. When Storm acks or fails a message,
    it will pass back to the spout the same message id to identify
    which tuple it's referring to.

    If the spout leaves out the message id, or sets it to null, then Storm
    ...
    def open(self, conf, context, collector):
        """ Called when a task for this component is initialized within
        a worker on the cluster.

        :param conf: The Storm configuration for this spout. This is the
        ...
        :param context: This object can be used to get information about
                this task's place within the topology, including the task id
                 and component id of this task, input and output information, etc.
        :param collector: The collector is used to emit tuples from this spout.
                Tuples can be emitted at any time, including the open and close
    ...
    def close(self):
        """ Called when a spout is going to be shutdown. There is no guarantee
    ...
    def activate(self):
        ...
        next_tuple will be called on this spout soon.
    ...
    def deactivate(self):
        """ Called when a spout has been deactivated. next_tuple will not be
    ...
    def ack(self, msg_id):
        """ Storm has determined that the tuple emitted by this spout with
        the msgId identifier has been fully processed. Typically, an
    ...
    def fail(self, msg_id):
        """ The tuple emitted by this spout with the msg_id identifier
        has failed to be fully processed. Typically, an implementation of
        this method will put that message back on the queue to be
